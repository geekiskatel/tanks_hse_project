...
FPS = 60
# Ширина и Высота изображения с текстурой (хитбокс)
TILE = 32
...
# Создание класса Танка
class Tank:
    # Создание таких параметров как цвет, координата по х и у, направление куда смотрит танк, кнопки управления
    ...
    # Функция получения урона и гибели танка
...
# Класс пули
class Bullet:
    # Создание таких параметров как танк, который выпускает, координаты полета (вектор), урон от неё
    def __init__(self, parent, px, py, dx, dy, damage):
        # Добавление пули в объекты
        bullets.append(self)
        # Звук выстрела
        sound_shot.play()
        # Указание танка владельца
        self.parent = parent
        # Указание координат
        self.px, self.py = px, py
        # Указание вектора полета
        self.dx, self.dy = dx, dy
        # Указание её урона
        self.damage = damage

    # Обновление состояний пуль
    def update(self):
        # Вектор по Х
        self.px += self.dx
        # Вектор по Y
...
class Bonus:
    ...
    # Отрисовка объекта с условием в зависимости от времени таймера
    def draw(self):
        # Условие для мигания бонуса
        if self.timer % 30 < 15:
            window.blit(self.image, self.rect)

# Класс логотипа ВШЭ
class HSE:
    # Указание координат и размера (32х32)
    def __init__(self, px, py, size):
        # Добавление в объекты
        objects.append(self)
        # Добавление типа
...
sound_theme.play()
# Этот объект будет обновлять данные
ui = UI()
